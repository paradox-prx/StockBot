    ...
    if (this.isLinked()) {
      if (realmKey) {
        ...
        if (!this.#currentHeadRealm) {
          ...
          /* if (!newHeadRealm) {
            console.log('missing realm', this, realmKey);
            debugger;
          }
          if (!oldHeadRealm) {
            console.log('missing realm', this, this.#currentHeadRealm);
            debugger;
          } */
...
class EntityTracker extends EventTarget {
  ...
  linkMap(realm, map) {
    ...
    if (added) {
      ...
      this.dispatchEvent(new MessageEvent('entitytransplant', {
        data: {
          arrayId: map.arrayId,
          entityId: map.arrayIndexId,
          entity: virtualMap,
...
class VirtualPlayersArray extends EventTarget {
  ...
  getKeys() {
    return Array.from(this.virtualPlayers.keys());
  }
  getValues() {
...
export class NetworkRealm extends EventTarget {
  ...
  async connect() {
    ...
    const ws1 = createWs(this.parent.endpointUrl, 'realm:' + this.key, this.parent.playerId);
...
export class NetworkRealms extends EventTarget {
  ...
  // - endpointUrl - The URL of the multiplayer server to connect to.
  ...
  if (!endpointUrl /* || !playerId || !audioManager*/) {
      console.warn('got bad network realms args', {
        endpointUrl,
        // playerId,
        // audioManager,
      });
    ...
    this.localPlayer = this.playerId ? new VirtualPlayer('players', this.playerId, this, 'local', {
    ...
    this.appsEntityTracker.addEventListener('entitytransplant', e => {
      if (!e.data.realm.parent.isUpdating()) {
        this.dispatchEvent(new MessageEvent('entitytransplant', {data: e.data}));
      }
    });
    ...
    this.addEventListener('realmleave', e => {
      ...
      const fn =this.realmsCleanupFns.get(realm);
      if (fn) {
        fn();
      ...
      // if no more realms, dispatchEvent disconnect
      // console.log('remaining size', this.connectedRealms.size);
      if (this.connectedRealms.size === 0) {
        this.dispatchEvent(new MessageEvent('disconnect', {
          data: {},
        }));
      }
  ...
  removeAudioSource(audioSource) {
    ...
    if (index !== -1) {
      ...
      const headRealm = this.localPlayer.headTracker.getHeadRealm();
      const {networkedAudioClient} = headRealm;
      networkedAudioClient.removeAudioSource(audioSource);
    } else {
  ...
  async updateRealmsKeys({
    realmsKeys = [],
    rootRealmKey = realmsKeys.length > 0 ? realmsKeys[0] : null,
  } = {}) {
    ...
    if (!arrayEquals(realmsKeys, this.lastRealmsKeys)) {
      ...
      await this.migrateTx(async () => {
        ...
        for (let i = 0; i < realmsKeys.length; i++) {
          ...
          realm.addEventListener('disconnect', e => {
            // const {
            //   errored,
            // } = e.data;
        ...
        for (const realm of candidateRealms) {
          ...
          if (!foundRealm) {
            ...
            const connectPromise = (async () => {
              ...
              this.localPlayer && this.localPlayer.link(realm);
        ...
        if (oldNumConnectedRealms === 0 && connectPromises.length > 0) {
          ...
          const e = new MessageEvent('connect', {
            data: {
              rootRealmKey,
            },
          });
        ...
        this.localPlayer && await this.localPlayer.headTracker.tryMigrate(rootRealmKey);
        ...
        for (const connectedRealm of this.connectedRealms) {
          if (!candidateRealms.find(candidateRealm => candidateRealm.key === connectedRealm.key)) {
            ...
            this.localPlayer && this.localPlayer.unlink(connectedRealm);
